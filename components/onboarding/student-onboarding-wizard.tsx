  ...
  async function onFinalSubmit(values: StudentRegistrationPayload) {
    ...
    try {
      ...
      addStudent({
        ...
        pin,
        membershipType: values.form_type === "renewal" ? "renewal" : "new",
        englishName: "",
  ...
  return (
    <main className="mx-auto max-w-lg space-y-6 p-6">
      <div className="flex items-center justify-between gap-4">
        <h1 className="text-xl font-semibold tracking-tight text-white">新學生登記 · F01</h1>
        <Link href="/student" className="text-sm text-zinc-400 underline underline-offset-4 hover:text-white">
          返回
        </Link>
      </div>

      <ol className="flex gap-2 text-xs text-zinc-500">
        <li className={step >= 1 ? "text-emerald-400" : ""}>① 個人／緊急聯絡</li>
        <li className={step >= 2 ? "text-emerald-400" : ""}>② PAR-Q</li>
        <li className={step >= 3 ? "text-emerald-400" : ""}>③ 冷靜期／簽署</li>
      </ol>
      ...
      >
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`rounded-lg px-3 py-2 text-sm ${
                  form.watch("form_type") === "new"
                    ? "bg-[#6366f1] text-white"
                    : "border border-white/[0.12] bg-transparent text-zinc-300"
                }`}
                onClick={() => form.setValue("form_type", "new")}
              >
                新人申請
              </button>
              <button
                type="button"
                className={`rounded-lg px-3 py-2 text-sm ${
                  form.watch("form_type") === "renewal"
                    ? "bg-[#6366f1] text-white"
                    : "border border-white/[0.12] bg-transparent text-zinc-300"
                }`}
                onClick={() => form.setValue("form_type", "renewal")}
              >
                Renewal 續會
              </button>
            </div>
            <input placeholder="姓名 *" {...form.register("full_name")} />
            {form.formState.errors.full_name && (
              <p className="text-xs text-rose-400">{form.formState.errors.full_name.message}</p>
            )}
            <input placeholder="HKID *（例：A1234567）" {...form.register("hkid")} />
            {form.formState.errors.hkid && (
              <p className="text-xs text-rose-400">{form.formState.errors.hkid.message}</p>
            )}
            <input placeholder="電話 * +852…" {...form.register("phone")} />
            {form.formState.errors.phone && (
              <p className="text-xs text-rose-400">{form.formState.errors.phone.message}</p>
            )}
            <input placeholder="電郵（可選）" {...form.register("email")} />
            <input placeholder="緊急聯絡人姓名 *" {...form.register("emergency_contact_name")} />
            <input placeholder="緊急聯絡人電話 *" {...form.register("emergency_contact_phone")} />
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            {PARQ_LABELS.map(({ key, label }) => (
              <Controller
                ...
                render={({ field }) => (
                  <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-white/[0.08] bg-[#121212] px-3 py-2 text-sm text-zinc-200">
                    <input
                      type="checkbox"
            ...
            {clearanceNeeded ? (
              <div className="space-y-2">
                <p className="text-sm font-medium text-amber-200">請上載醫療／醫生 clearance</p>
                <input
                  type="file"
                  accept=".pdf,.jpg,.jpeg,.png"
                  className="text-xs text-zinc-400 file:mr-3 file:rounded-md file:border-0 file:bg-[#6366f1] file:px-3 file:py-1.5 file:text-white"
                  onChange={(e) => {
              ...
              <p className="text-xs text-zinc-500">無需額外 clearance。</p>
            )}
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <div>
              <label className="text-xs text-zinc-500">方案</label>
              <select className="mt-1" {...form.register("package_sessions", { valueAsNumber: true })}>
                <option value={10}>10 堂（有效期 3 個月 · 預覽到期 {expiryPreview}）</option>
                <option value={30}>30 堂（有效期 6 個月 · 預覽到期 {expiryPreview}）</option>
              </select>
            </div>
            <input placeholder="電子簽署：輸入全名 *" {...form.register("digital_signature")} />
            ...
            <textarea placeholder="續會備註（可選）" rows={2} {...form.register("renewal_notes")} />
          </div>
        )}
        ...
        <div className="flex flex-wrap gap-2 border-t border-white/[0.08] pt-4">
          ...
          {step < 3 ? (
            <button type="button" className="bg-[#6366f1] text-white hover:bg-[#535bf0]" onClick={() => void goNext()}>
              下一步
            </button>
          ) : (
