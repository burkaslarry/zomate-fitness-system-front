/**
 * [F004][S005]
 * Demo: Admin payroll export — filters, preview table, student package popup.
 * Run: E2E_BASE_URL=https://zomate-fitness-system-front.vercel.app PLAYWRIGHT_SKIP_WEBSERVER=1 npx playwright test payroll-export-demo-recording.spec.ts
 */

import { test, expect } from "@playwright/test";

test.use({
  viewport: { width: 1280, height: 720 },
  video: { mode: "on", size: { width: 1280, height: 720 } },
  launchOptions: { slowMo: 350 }
});

test("record coach attendance income export demo", async ({ page }) => {
  test.setTimeout(120_000);

  await page.goto("/login?logout=1");
  await page.getByLabel("帳號").fill("masterzoe");
  await page.getByLabel("密碼").fill("12345678");
  await page.getByRole("button", { name: "登入", exact: true }).click();
  await page.waitForURL("**/admin**", { timeout: 30_000 });
  await page.waitForTimeout(1200);

  await page.goto("/admin/finance/payroll");
  await expect(page.getByRole("main").getByRole("heading", { name: "教練出勤收入匯出" })).toBeVisible({
    timeout: 20_000
  });
  await page.waitForTimeout(2000);

  const coachSelect = page.locator("select").first();
  const options = coachSelect.locator("option");
  const optionCount = await options.count();
  if (optionCount > 1) {
    await coachSelect.selectOption({ index: 1 });
    await page.waitForTimeout(2500);
  }

  await page.getByLabel("開始日期").fill("2026-07-01");
  await page.getByLabel("結束日期").fill("2026-08-31");
  await page.waitForTimeout(3000);

  const studentLink = page.locator("tbody button").first();
  if (await studentLink.isVisible().catch(() => false)) {
    await studentLink.click();
    const dialog = page.getByRole("dialog");
    await expect(dialog).toBeVisible({ timeout: 10_000 });
    await page.waitForTimeout(2500);
    await dialog.getByRole("button", { name: "關閉" }).last().click();
    await page.waitForTimeout(800);
  }

  await page.getByRole("button", { name: "匯出 CSV" }).scrollIntoViewIfNeeded();
  await page.waitForTimeout(1500);
});
